...
RapidX CLI (`@liquiditytech/rapidx-cli`) provides two **independent** integration paths — choose either one or both:
...
Log in to the competition platform at **https://www.liquiditytech.com/** (Phase II Mainnet / Track B RapidX) or **https://uat.liquiditytech.com/** (Phase I Sandbox) and create your API credentials under **Profile → API Management → Generate**. This produces the `LTP_ACCESS_KEY` / `LTP_SECRET_KEY` pair used below.

Each account includes:
- `LTP_ACCESS_KEY` and `LTP_SECRET_KEY` — your API credentials (from API Management)
- `LTP_API_HOST` — Phase I (Sandbox): `https://api.ltp-contest.com` · Phase II (Mainnet): `https://api.liquiditytech.com`
- Initial capital: **1,000 USDT** (no real assets at risk)
...
> **Account Restrictions**: Once the main competition begins, self-service deposits and withdrawals are disabled — the deposit API is banned. Transfers between Binance and OKX within RapidX are permitted, as all teams trade exclusively on RapidX. During the Phase II preparation phase, teams provide their deposit address to the Organizing Committee, who funds the account with 1,000 USDT on their behalf.

> See [`resources.md §1`](./resources.md#1-competition-account) for full account details and Track A symbol restrictions.

> ⚠️ **Key competition rules**: Elimination is triggered when **Equity < 800 USDT**. Agent uptime must be ≥ 90%. See [`resources.md §15`](./resources.md#15-safety--compliance-reminders).
...
# Phase II (Mainnet) — use https://api.liquiditytech.com
# Phase I (Sandbox) — use https://api.ltp-contest.com
...
| [`04-news-feed.md`](./04-news-feed.md) | News Feed REST + WebSocket — real-time and historical crypto news |
