# Leaderboard API (Track A)

Query your own team's ranking and scoring snapshot for a Track A competition phase. The `portfolioId` is derived server-side from your API Key — do not pass it.

> For the Track B equivalent, see [Track B Ranking — Self Query](trackb-ranking-self.md).
